...
- `AGENT_USAGE_URL` overrides the endpoint, e.g. `https://blog.localhost/api/usage/ingest` for the local dev server (defaults to `VERCEL_PROJECT_PRODUCTION_URL`, `VERCEL_URL`, then `https://ruchern.dev`)
- `AGENT_USAGE_DRY_RUN=1` prints the payload without POSTing
