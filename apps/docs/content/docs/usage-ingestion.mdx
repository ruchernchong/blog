...
The Rust collector in `packages/usage` parses local agent logs and POSTs daily rows to the ingest endpoint.
...
- `packages/usage` parses, folds, and posts token rows.
