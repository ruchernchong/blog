...
The body must match `UsageIngestPayload` from `apps/web/src/lib/usage/ingest.ts`. Field names are **camelCase** (matching AgentUsage Codable and the token-row wire style).
...
The Rust collector in `packages/usage/rust` parses local agent logs and POSTs daily rows to the ingest endpoint.
...
Sign in once per server (OAuth, admin account; the token is stored in the Keychain). Login follows `USAGE_INGEST_URL`, so a local dev server and production keep separate logins. The installed collector is production only. Rows are sent with `costUsd: null`, and the route prices them after the model registry sync. The collector emits token rows only; effort rows come from AgentUsage.

Optional environment variables:

- `USAGE_INGEST_URL` overrides the endpoint, e.g. `https://blog.localhost/api/usage/ingest` for the local dev server (defaults to `VERCEL_PROJECT_PRODUCTION_URL`, `VERCEL_URL`, then `https://ruchern.dev`)
- `USAGE_INGEST_DRY_RUN=1` prints the payload without POSTing
...
- `apps/web/src/lib/usage/ingest.ts` defines the wire schema.
- `packages/usage/rust` parses, folds, and posts token rows.
