...
The response contains an `access_token` (JWT when `resource` was passed), and a `refresh_token` if `offline_access` was requested.
...
2. **OAuth access token** — the JWT is verified locally against the provider's JWKS via `serverClient.verifyAccessToken` (`apps/web/src/lib/server-client.ts`), with `audience` set to `NEXT_PUBLIC_BASE_URL`. The owning user and role are then loaded by the token `sub`.
3. **Static MCP token** — the legacy `BLOG_MCP_AUTH_TOKEN` bearer, retained for headless clients until they migrate to OAuth.
...
- Pass `resource=https://ruchern.dev` (or the target API base URL) at authorize so the access token is a verifiable JWT.
