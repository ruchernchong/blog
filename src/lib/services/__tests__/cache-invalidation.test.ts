...
import * as postTagsQueries from "@/lib/queries/post-tags";
...
describe("cache-invalidation", () => {
  ...
  describe("invalidateRelatedByTags", () => {
    it("should invalidate related caches for all posts with overlapping tags", async () => {
      ...
      vi.mocked(postTagsQueries.getPostsWithOverlappingTags).mockResolvedValue(
        mockPosts as any,
      );
      vi.mocked(redis.del).mockResolvedValue(undefined as any);
      ...
      expect(postTagsQueries.getPostsWithOverlappingTags).toHaveBeenCalledWith(
        ["typescript", "react"],
        "",
      );
    ...
    it("should do nothing when no tags provided", async () => {
      ...
      expect(
        postTagsQueries.getPostsWithOverlappingTags,
  ...
  describe("invalidatePopularPost", () => {
    ...
    it("should run operations in parallel", async () => {
      ...
      vi.mocked(redis.zrem).mockReturnValue(removePromise as any);
